...
async function handleWatchlistActionFeedback(card, mediaId, mediaType, newStatus, previousStatus) {
    ...
    if (isLibraryPage) {
        // On the library page, fade out the card and re-render the list
        if (card) {
            ...
            // Fallback if card isn't available, just re-render
            const appRoot = document.getElementById('app-root');
            const currentActiveFilter = appRoot.querySelector('.chip.active')?.dataset.filter || 'watchlist';
            const { renderLibraryPage } = await import('./watchlist-page.js');
            renderLibraryPage(appRoot, { activeFilter: currentActiveFilter });
        ...
        // On the person page filmography, just show a simple toast and don't hide the card.
        showToast(toastOptions);
    } else {
        // On discovery pages, hide the card and provide an "Undo" option.
        const shouldHideCard = newStatus !== 'remove';
        if (shouldHideCard && card) {
            ...
            // Fallback for removals or if card isn't available.
            showToast(toastOptions);
        }
    }
}
...
appRoot.addEventListener('click', async (event) => {
    ...
    if (button) { // An action button was clicked
        ...
        // The card itself was clicked for navigation
        window.location.hash = `#media?type=${mediaType}&id=${mediaId}`;
    }
});