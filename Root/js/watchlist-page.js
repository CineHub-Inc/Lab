...
function initSortableGrid(gridContainer, appRoot) {
    ...
    new Sortable(gridContainer, {
        ...
        onEnd: async function (evt) {
            const orderedCards = [...evt.to.querySelectorAll('.media-card')];
...
export async function renderLibraryPage(appRoot, params = {}) {
    ...
    try {
        ...
        const gridHtml = sortedItems.map(([itemString, itemData]) => {
            ...
            const cardHtml = createMediaCard(media);
            
            // Make card draggable and add status/title data attributes for filtering
            return cardHtml.replace(
                'class="media-card"', 
                `class="media-card" draggable="true" data-status="${itemData.status}" data-title="${title}"`
            );
        }).join('');
