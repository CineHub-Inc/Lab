...
function renderFilmographyGrid(grid, credits, personData) {
    ...
    const filmographyHtml = credits.map(credit => {
        const cardHtml = createMediaCard(credit);
        return cardHtml.replace(
            /class="media-card"/,
            `class="media-card" style="cursor: default;"`
        ).replace(
            /<div class="media-card" data-id="(.+?)" data-type="(.+?)" data-title="(.+?)">/,
            `<a href="#media?type=${credit.media_type}&id=${credit.id}${fromParams}" class="media-card" data-id="${credit.id}" data-type="${credit.media_type}" data-title="${credit.title || credit.name}">`
        ).replace(/<\/div>$/, '</a>');
    }).join('');
...
function setupFilmography(personData, creditsData) {
    ...
    const uniqueCredits = allCredits.reduce((acc, current) => {
        ...
        const hasNoGenres = !current.genre_ids || current.genre_ids.length === 0;
        if (current.poster_path && !acc.find(item => item.id === current.id) && !hasExcludedGenre && !hasNoGenres) {
            acc.push(current);
        }
        return acc;
    }, []);
    ...
    const films = uniqueCredits.filter(c => c.media_type === 'movie');
    const series = uniqueCredits.filter(c => c.media_type === 'tv' && c.character !== 'Self');

    renderFilmographyGrid(document.getElementById('filmography-films-grid'), films, personData);
    renderFilmographyGrid(document.getElementById('filmography-series-grid'), series, personData);
}
...
function setupPageEventListeners(personData) {
    ...
    if (filmographySection) {
        filmographySection.addEventListener('click', (e) => {
            if (e.target.closest('a.media-card')) {
